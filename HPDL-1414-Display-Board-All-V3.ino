...
// transfer data from disptext to SRAM buftmp_2
  for (n=0;n<20;n++) {buftmp_2[n]=pgm_read_byte_near(disptext+n);}
  displayRamString(buftmp_2);
}

char scrollInString(char disptext[maxSize])
{
  // transfer data from disptext to SRAM buftmp_2
  for (n=0;n<20;n++) {buftmp_2[n]=pgm_read_byte_near(disptext+n);}
  ...
  for (o=0; o<20; o++)
    {
      ...
      for (p=0; p<20; p++) {buftmp_1[p]=buftmp_1[p+1];}
  ...
  //keep buffers empty
  for (p=0; p<20; p++) {buftmp_1[p]=char(0x20);buftmp_2[p]=char(0x20);}
}

char scrollOutString(char disptext[maxSize])
{
  // transfer data from disptext to SRAM buftmp_1
  for (n=0;n<20;n++) {buftmp_1[n]=pgm_read_byte_near(disptext+n);}
  // execute shift left complete buftmp_1
  for (o=0; o<20; o++)
    {
      // shift left buftmp_1 1 position <-
  ...
  //keep buffers empty
  for (p=0; p<20; p++) {buftmp_1[p]=char(0x20);}
}

char splitString(char disptext[maxSize])
{
  // transfer data from disptext to SRAM buftmp_2
  for (n=0;n<20;n++) {buftmp_1[n]=pgm_read_byte_near(disptext+n);}
  displayRamString(buftmp_1);
  ...
  for (o=0; o<10;o++)
  {
    for (p=0; p<10;p++) {buftmp_1[p]=buftmp_1[p+1];buftmp_1[19-p]=buftmp_1[19-p-1];}
